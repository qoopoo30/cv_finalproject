...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    for m = 1:N1
        d1_temp = repmat(descriptor1(m, :), N2, 1);
        Euclidean = sqrt(sum((d1_temp - descriptor2).^2, 2));   % Euclidean is a N2 x 1 matrix
        E_temp = sort(Euclidean);
        if E_temp(1) < E_temp(2)* thresh
            match = [match; m, find(Euclidean == min(Euclidean))];
        end
    end
